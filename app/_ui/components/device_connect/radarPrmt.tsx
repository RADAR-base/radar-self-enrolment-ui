...
function MobileContent({prmtAuthUrl}: {prmtAuthUrl: string}) {
  return <React.Fragment>
    ...
    <Grid size={{xs: 12, sm: 6}}>
      <Button href={prmtAuthUrl} variant={"contained"}>Open Study App</Button>
    </Grid>
  </React.Fragment>
}
...
export function PrmtPage() {
  ...
  const [prmtAuthUrl, setArmtAuthUrl] = useState<any>(undefined)


  if ((code == undefined) && (prmtAuthUrl == undefined) && (isFetchingToken == false)) {
    router.replace(`/connect/prmt?return_to=/${studyId}/portal/connect/radar_prmt`)
  }
  
  useEffect(() => {
    const handleToken = async () => {
      if (isFetchingToken || (prmtAuthUrl != undefined)) return
      if (code) {
    ...
    <RadarCard>
      <Grid container spacing={2} padding={3} gap={2} rowGap={4}>
        ...
        <Grid size={{xs: 12, sm: 6}} textAlign={'left'}>
          ...
          <Typography variant="body1">This study uses the RADAR pRMT app to access phone sensor data. You will need to download the app from the Apple App Store or Google Playstore.</Typography>
        </Grid>
        <Grid size={{xs: 12, sm: 6}}>
        ...
        {(isMobile || isTablet) && <MobileContent prmtAuthUrl={prmtAuthUrl} />}
        <Grid size={{xs: 12, sm: 6}} textAlign={'left'}>
        ...
        <Grid size={{xs: 12, sm: 6}}>
          ...
          {prmtAuthUrl && <QRCodeSVG value={prmtAuthUrl} size={200} />}
          <NextButton href={`/connect/prmt?return_to=${pathname}`}>Generate QR Code</NextButton>
          </Box>
        </Grid>
        <Grid size={{xs: 12, sm: 6}} textAlign={'left'}>
